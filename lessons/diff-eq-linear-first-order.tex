...
Equations~\eqref{eq:diff-eq-integrating-factor} and \eqref{eq:diff-eq-linear-first-order-solution1} both come out of nowhere, and Equation~\eqref{eq:diff-eq-linear-first-order-solution1} looks rather complicated. The good news is that Theorem~\ref{thm:diff-eq-linear-first-order} is the compact form of Method~\ref{method:integrating-factor} on page~\pageref{method:integrating-factor}.
...
In real-life applications, we often wish to understand the long-term behaviour of our models.
\begin{example}
