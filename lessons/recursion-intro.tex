...
\hlmain{Discrete-time models} are models in which the time variable takes on (typically non-negative) integer values. In discrete-time models, time progresses in integer-valued steps instead of flowing continuously. 

\hlmain{Recursions}, also known as difference equations, are equations for describing changes over discrete time steps and are often used to describe discrete-time models.
...
\begin{example} \label{ex:origami}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics{../standalones/demo-origami}
  \end{figure}
...
\begin{definition}[recurrence]
  Let \(u_{t}\) be a discrete-time model, i.e., time \(t\) are non-negative integers.  We call \(u_{t}\) the \hlmain{state at time \(t\)}.

  The model \(u_{t}\) is described by a \hlmain{recurrence} when two things are specified:
  \begin{enumerate}
    \item An \hlmain{initial condition} (or initial state) \(u_{0}\).
    \item A \hlmain{recurrence equation} that relates \hlwarn{Edited: ``relate'' \(\to\) ``relates''} \(u_{t}\) to one or more \emph{previous} states.
  \end{enumerate}
