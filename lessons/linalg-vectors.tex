...
We have been looking at models involving one quantity, e.g., the population of a group of unicorns.  We now develop mathematical tools (vectors and matrices) \hlwarn{Edited: ``matricies'' \(\to\) ``matrices''} to model systems involving two or more quantities, e.g., an ecosystem involving predators and prey. \hlwarn{Edited: ``preys'' \(\to\) ``prey''}

We use an introductory example to demonstrate why vectors are useful in science.
\begin{example}
  Consider an ecosystem involving orcas (predators) and seals (prey). \hlwarn{Edited: ``preys'' \(\to\) ``prey''} Their population over time (in years) is presented as a table. 
...
\begin{definition}[vectors]
  ...
  The number of rows in \(\vec{x}\) (zeros included) is called the \hlmain{dimension} (or the \hlmain{length}) of \(\vec{x}\).  Two vectors \(\vec{u}, \vec{v}\) are \hlmain{equal} if they have the same dimension and \(u_{1} = v_{1}, \ldots, u_{n} = v_{n}\). \hlwarn{Edited: ``\ldots.'' \(\to\) ``\ldots'' (stray period)}
\end{definition}
...
\begin{itemize}[wide]
  ...
  \item We can multiply a \hlsupp{constant \(c\)} by a \hlmain{vector \(\vec{v}\)}, and \hlmain{add two vectors} having the same dimension.
    \begin{equation}
      ...
      \end{bmatrix}.
    \end{equation}
    \hlwarn{Edited: in the sum above, the second entry ``\(v_{2} + v_{2}\)'' \(\to\) ``\(u_{2} + v_{2}\)''} \hlwarn{FIXME: verify this math/content correction}

    \blanklines{5}

    \includegraphics{../standalones/plot-RR2}
    \quad
    \includegraphics{../standalones/plot-RR2}
    \quad
    \includegraphics{../standalones/plot-RR2}
...
In general, we can multiply an \(m\)-by-\(n\) matrix \(A\) by an \(n\)-dimensional vector \(\vec{v}\) to produce an \(m\)-dimensional vector \(\vec{u} = A\vec{v}\) whose entries are defined by
\[
  u_{i} = A_{i1} v_{1} + \cdots + A_{in} v_{n}
  \quad\text{for each row index \(i = 1, \ldots, m\)}.
\]
\hlwarn{Edited: an \(m\)-by-\(n\) matrix times an \(n\)-vector yields an \(m\)-vector, so ``\(n\)-dimensional vector \(\vec{u}\)'' \(\to\) ``\(m\)-dimensional'' and the row index ``\(i=1,\ldots,n\)'' \(\to\) ``\(i=1,\ldots,m\)''; also ``a \(n\)-dimensional'' \(\to\) ``an \(n\)-dimensional''} \hlwarn{FIXME: verify this math/content correction}

\blanklines{5}

% The number of columns in \(A\) \hlwarn{must match} the number of rows in \(\vec{v}\).  Matrix-vector multiplication is defined by 
A main reason we study matrices \hlwarn{Edited: ``matricies'' \(\to\) ``matrices''} is that matrix-vector multiplication can be used to encode linear equations in variables \hlwarn{Edited: ``variable'' \(\to\) ``variables''} \(x_{1}, \ldots, x_{n}\) in the form \(A \vec{x} = \vec{b}\).
...
Identity matrices \hlwarn{Edited: ``matricies'' \(\to\) ``matrices''} are special because \hlmain{\(I_{n} A = A\) and \(A I_{n} = A\)} \hlwarn{Edited: second ``\(I_{n} A = A\)'' \(\to\) ``\(A I_{n} = A\)''} \hlwarn{FIXME: verify this math/content correction} as long as \(A\) is an \(n \times n\) matrix. Moreover, \(I_{n} \vec{v} = \vec{v}\) as long as \(\vec{v}\) has \(n\) rows. In other words, \hlsupp{multiplication} by an identity matrix does nothing.
