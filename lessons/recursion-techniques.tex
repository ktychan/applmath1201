...
\begin{thm} \label{thm:recurrence-exp}
  Let \(a,\lambda\) be constants and \(\lambda > 0\). The solution to the recurrence
  \[ 
    u_{t} = \lambda u_{t-1}, \text{ for integers } t \ge 1 
  \]
  is 
  \[
    u_{t} = u_{0} \lambda^{t}.
...
\begin{method} \label{method:recurrence-sub}
  Given a recurrence of the form \(v_{t} = A v_{t-1} + B\), we wish to make a change of variable \(u_{t} = v_{t} + c\) so that \(u_{t}\) is the recurrence in Theorem~\ref{thm:recurrence-exp}. Solve the new recurrence and recover the original solution.
\end{method}
...
\begin{example}[retirement savings]
  How much savings does one need at the time of retirement (say, January 1, 2060) if the savings generate \(5\%\) return on investment annually, and they withdraw \(\$150,000\) at the beginning of each year.
