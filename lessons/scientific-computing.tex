...
f(3)
    \end{pythoncode}
    The above performs the calculation \(f(3)\). We expect the output to be \(3^{2} = 9\).
...
\begin{enumerate}
  \item Functions defined using numpy commands are capable of calculating a bunch of values very quickly.  For example, we wish to calculate
    \[
      L_{0} = g(2), \quad L_{1} = g(4), \quad L_{2} = g(6), \quad L_{3} = g(8).
    \]

    Instead of writing \pythoninline{L = [ g(2), g(4), g(6), g(8) ]} (which works), we can write this:
    \begin{pythoncode}
...
print(L)
  ...
  \item Computers plot graphs by literally plotting point by point and connecting the dots by straight lines.  Therefore, plotting graphs requires sampling a function at a bunch of \(x\)-values. 

    The command \pythoninline{numpy.linspace(start, stop, num)} quickly generates a list of evenly spaced \(x\)-values for plotting.

    For example, the list of numbers \(-1, 0, 1\) is evenly spaced that starts at \(-1\), ends at \(1\), and has \(3\) numbers in total.  Instead of writing a for-loop, we simply write the following.
    \begin{pythoncode}
