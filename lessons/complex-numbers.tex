...
For any complex number \emph{simplified} to the form \(z = a + bi\) where \(a,b\) are \underline{\hspace{3cm}}, define its \hlmain{complex conjugate} to be \(\bar{z} = \underline{\hspace{1in}}\) and its \hlmain{length} to be \(|z| = \underline{\hspace{3cm}}\).  It follows that division has a shorter formula
\[
  \frac{1}{z} = \hspace{1in}
\]
...
We can visualize algebraic operations for complex numbers because they can always be simplified to the form \(a + bi\) where \(a,b\) are real numbers.

\begin{example}
  Visualize \((2 + 3i) + (-1 + i)\) and \(i (4 + i)\).

  \begin{center}
  \includegraphics{../standalones/plot-complex-plane}
  \quad
  \includegraphics{../standalones/plot-complex-plane}
  \end{center}
\end{example}
