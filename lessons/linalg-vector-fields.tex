...
However, we should understand that \emph{linear vector fields} are really just a fancy term for ``\emph{\hlattn{change} matrices}'' \hlwarn{Edited: ``matricies'' \(\to\) ``matrices''} which will come up again when we discuss systems of differential equations.

\blanks{20}

\clearpage
Vector fields \hlwarn{Edited: ``Vectors fields'' \(\to\) ``Vector fields''} visualize motion encoded by recurrence equations \(\vec{x}(t+1) = A \vec{x}(t)\).  If we were to visually solve such a recurrence, we simply follow the arrows starting at some initial condition \(\vec{x}(0)\).
...
\begin{exercise}
  ...
  \begin{center}
    \includegraphics{../standalones/plot_vector_field_rotation2}
    \quad
    \includegraphics{../standalones/plot_vector_field_rotation3}
  \end{center}
\end{exercise}
\vfill{}
...
\begin{example}
  ...
  \begin{center}
    \includegraphics{../standalones/plot_vector_field_intro}
    \quad
    \includegraphics{../standalones/plot_vector_field_intro_scaled}
  \end{center}
\end{example}
\vfill{}
\clearpage
...
\begin{exercise}
  ...
  Use both vector fields \hlwarn{Edited: ``both vector field'' \(\to\) ``both vector fields''} below to sketch the long-term behaviour for a recurrence with initial condition \(\vec{x}(0) = \begin{bmatrix} 80 \\ 0 \end{bmatrix}\). Are the results really different?

  \begin{center}
    \includegraphics{../standalones/plot_vector_field_rotation}
    \quad
    \includegraphics{../standalones/plot_vector_field_rotation_scaled}
  \end{center}
\end{exercise}
\end{document}
