...
\begin{example}[Lotka-Volterra model]
  ...
  The dependent variables (unknown functions) in the Lotka-Volterra equations are \underline{\hspace{1in}}, and the independent variable is \underline{\hspace{1cm}}.
\end{example}
...
Classification of systems of DEs is similar \hlwarn{Edited: ``are similar'' \(\to\) ``is similar''} to that of differential equations. However, notice the use of ``only'' and ``some'' in the definitions below.
\begin{itemize}
...
For example, the Lotka-Volterra \hlwarn{Edited: ``Lokta-Volterra'' \(\to\) ``Lotka-Volterra'' (all occurrences in this section)} equations are non-linear, first-order differential equations.
\clearpage
