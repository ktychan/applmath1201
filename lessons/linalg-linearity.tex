...
We learn a bit more theory of vectors and matrices \hlwarn{Edited: ``matricies'' \(\to\) ``matrices'' (all occurrences in this section)} to prepare for solving systems of recurrence equations and systems of differential equations.
...
\begin{definition}[linearity] \label{def:linearity}
  A function \(F(\vec{x})\) \hlwarn{Edited: ``\(f(\vec{x})\)'' \(\to\) ``\(F(\vec{x})\)'' to match \(F(\vec{x}) = A\vec{x}\)} is called \hlmain{linear} if \(F(\vec{x}) = A\vec{x}\) for some matrix \(A\). If a function is not a matrix-vector multiplication, then it is not linear. All linear functions satisfy the property
...
Consider a matrix \(A = \begin{bmatrix} 2 & -3/2 \\ 0 & -1 \end{bmatrix}\) and its two eigenvectors \(\vec{u} = \begin{bmatrix} 1 \\ 2 \end{bmatrix}\) and \(\vec{v} = \begin{bmatrix} 1 \\ 0 \end{bmatrix}\).  Our goal is to find \hlmain{a simple answer} for arbitrary powers \(A^{n} \vec{x}\) where \(\vec{x} = \begin{bmatrix} -11 \\ 10 \end{bmatrix}\).
