...
\pgfplotsset{
  loglog line/.style={
    ...
    ymin=-2, ymax=2,
    xmin=-2, xmax=2,
    xlabel style = {anchor=south, font={\tiny}},
...
\chapter*{Week 1 practice problems}
...
\begin{exercise}
  ...
  \begin{enumerate}
    \item If heart rate increases by \(5\%\), how does metabolic rate change in terms of scaling and percentage?
      \begin{solution}
        The model is \(R \propto m^{3/4}\). A \(5\%\) increase means we scale mass by \(1.05\). In terms of scaling, heart rate scales \emph{by a factor} of \(1.05^{3/4} \approx 1.037\) which is an increase. In terms of percentage, metabolic rate increase by roughly \(3.7\%\).
      \end{solution}

    \item If heart rate decreases by \(7\%\), how does metabolic rate change in terms of scaling and percentage?
      \begin{solution}
        The model is \(R \propto m^{3/4}\). A \(7\%\) decrease means we scale mass by \(0.93\). In terms of scaling, metabolic rate scales \emph{by a factor} of \(0.93^{3/4} \approx 0.962\) which is an decrease. In terms of percentage, heart rate decreases by roughly \(1 - 0.962 = (3.8\%)\).
      \end{solution}

    \item If body mass increases by \(10\%\), how does heart rate change in terms of scaling and percentage?
      \begin{solution}
        The model is \(R \propto m^{-1/4}\). A \(10\%\) increase means we scale mass by \(1.1\). In terms of scaling, metabolic rate scales \emph{by a factor} of \(1.1^{-1/4} \approx 0.976\) which is an decrease. In terms of percentage, heart rate decreases by roughly \(1 - 0.976 = (2.4\%)\).
      \end{solution}

    \item If body mass decreases by \(5\%\), how does heart rate change in terms of scaling and percentage?
      \begin{solution}
        The model is \(R \propto m^{-1/4}\). A \(5\%\) decrease means we scale mass by \(0.95\). In terms of scaling, metabolic rate scales \emph{by a factor} of \(0.95^{-1/4} \approx 1.0129\) which is an increase. In terms of percentage, heart rate increases by roughly \(1.29\%\).
      \end{solution}
